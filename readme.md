...
안녕하세요. **강남대학교 AI융합공학부 인공지능 전공 이승윤**입니다.  
저는 단순히 모델 성능만 높이는 것보다,  
...
해외 리뷰, 트렌드, 수출 관련 데이터를 결합해  
국가별 K-Food 수요를 예측하고,  
현지화 추천과 자동 리포트까지 연결하는 플랫폼입니다.

**주요 기여**
- 수요·트렌드 예측 모델 개발
- **XGBoost → LightGBM 전환** 및 성능 개선
- 국가별 **TOP-N 추천 리포트 파이프라인** 구성
- **ABSA 기반 VOC 정량화**
- JSON 스키마 표준화
- 도움말 챗봇 및 레시피 챗봇 연결
...
### 2) AI 면접관 Agent
이력서/자기소개서를 기반으로  
질문 생성 → 답변 평가 → 추가 질문 → 인터뷰 리포트 자동화까지 수행하는 Agent입니다.

**주요 기여**
- LangGraph 기반 상태형 인터뷰 플로우 설계
- 이력서 요약 / 질문 전략 생성
- 답변 평가 및 후속 질문 생성
- 최종 인터뷰 리포트 자동 작성
...
### 3) AI 강사 Agent
강의자료를 입력하면  
수업 설계, 요약, 퀴즈, 과제, 피드백을 자동 생성하는 Agent 프로젝트입니다.
...
### 4) 광고 없는 맛집 탐색 앱
주소 기반으로 주변 음식점을 탐색하고,  
카카오맵/네이버 후기 등을 활용해 광고성 글을 최대한 걸러내는 맛집 탐색 앱입니다.

**주요 기여**
- 팀장 역할 수행
- 주소 기반 탐색 및 위치 정확도 개선
- 카카오맵 / 네이버 후기 연동
- 광고 키워드 필터링
- 추천 점수 로직 설계
...
### 5) 스포츠 기사 감성분석
스포츠 기사 텍스트를 기반으로 감성을 분류하는 NLP 프로젝트입니다.
...
### 6) 인공지능 작가 (개인 프로젝트)
React Native와 OpenAI GPT-3.5를 활용해  
사용자 맞춤형 스토리를 생성하는 개인 앱 프로젝트입니다.
...
### 7) 미로 탈출 게임 (팀장)
미로, 퀴즈, 힌트, 스도쿠, 공포 요소를 결합한 게임 프로젝트입니다.
...
### 8) 미니 악당 딥러닝 프레임워크 (팀장)
캐릭터 말투 유지와 데이터셋 관리 모듈을 포함한  
확장형 딥러닝 프레임워크 프로젝트입니다.
...
![LightGBM](https://img.shields.io/badge/LightGBM-0C7C59?style=for-the-badge)
